...
# ---- daily quota accounting (free tier burns 25 calls/day) ----

def _call_once(monkeypatch):
    monkeypatch.setattr(av.requests, "get", _patched_get("Date,Close\n2025-01-02,1.0"))
    av._make_api_request("TIME_SERIES_DAILY", {"symbol": "AAPL"})


@pytest.mark.unit
def test_quota_counter_tracks_calls_per_day(monkeypatch):
    av._reset_quota_usage()
    _call_once(monkeypatch)
    _call_once(monkeypatch)
    count, _day = av.get_quota_usage()
    assert count == 2


@pytest.mark.unit
def test_quota_counter_warns_near_limit(monkeypatch, caplog):
    import logging
    av._reset_quota_usage()
    # Push the counter just under the warning threshold, then one more call
    # should cross it and emit a visible warning.
    monkeypatch.setattr(av, "_usage_count", av.DAILY_QUOTA_WARNING_THRESHOLD - 1)
    monkeypatch.setattr(av, "_usage_date", date.today().isoformat())
    with caplog.at_level(logging.WARNING):
        _call_once(monkeypatch)
    assert any("Alpha Vantage" in rec.message for rec in caplog.records)


@pytest.mark.unit
def test_quota_counter_resets_on_new_day(monkeypatch):
    av._reset_quota_usage()
    _call_once(monkeypatch)
    # Simulate a calendar rollover: the next call must restart the count at 1.
    monkeypatch.setattr(av, "_usage_date", "1999-01-01")
    _call_once(monkeypatch)
    count, _day = av.get_quota_usage()
    assert count == 1
