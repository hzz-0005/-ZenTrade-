...
def test_ensure_api_key_never_overwrites_parent_env(monkeypatch, tmp_path, cli_utils):
    """Writing a prompted secret must be scoped to the directory the user ran."""
    monkeypatch.delenv("DEEPSEEK_API_KEY", raising=False)
    parent_env = tmp_path / ".env"
    parent_env.write_text("DEEPSEEK_API_KEY=real-parent-value\n", encoding="utf-8")
    child = tmp_path / "isolated-run"
    child.mkdir()
    monkeypatch.chdir(child)
    fake_prompt = type("P", (), {"ask": staticmethod(lambda: "temporary-child-value")})()

    with patch.object(cli_utils.questionary, "password", return_value=fake_prompt):
        cli_utils.ensure_api_key("deepseek")

    assert "real-parent-value" in parent_env.read_text(encoding="utf-8")
    assert "temporary-child-value" not in parent_env.read_text(encoding="utf-8")
    assert "temporary-child-value" in (child / ".env").read_text(encoding="utf-8")
