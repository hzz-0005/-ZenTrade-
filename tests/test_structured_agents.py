...
@ pytest.mark.unit
class TestPriceLikeAndActionCoercion:
    """A weak LLM may write a descriptive Chinese sentence into an optional
    price field or the action enum (#1066); keep descriptive text so the
    structured call validates instead of erroring and double-spending tokens."""

    def test_trader_descriptive_stop_loss_kept_as_string(self):
        p = TraderProposal(
            action=TraderAction.HOLD,
            reasoning="x",
            stop_loss="跌破20日线止损离场",
        )
        assert p.stop_loss == "跌破20日线止损离场"

    def test_trader_action_chinese_buy(self):
        p = TraderProposal(action="买入，分批建仓", reasoning="x")
        assert p.action is TraderAction.BUY

    def test_trader_action_chinese_hold(self):
        p = TraderProposal(action="建议继续持有观望，等待企稳信号", reasoning="x")
        assert p.action is TraderAction.HOLD

    def test_trader_action_chinese_sell(self):
        p = TraderProposal(action="破位清仓离场", reasoning="x")
        assert p.action is TraderAction.SELL

    def test_trader_action_ambiguous_left_for_fallback(self):
        # Names two directions -> left untouched -> the free-text fallback
        # (and its deterministic rating parser) resolves it instead.
        with pytest.raises(ValidationError):
            TraderProposal(action="先持有，破位则卖出", reasoning="x")
