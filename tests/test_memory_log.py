...
class TestPortfolioManagerInjection:
    ...
    def test_pm_delivers_account_state_as_tool_result(self):
        """With account_state present, the PM hands it to the LLM as the RESULT
        of a get_account_state tool call — not as prose inside the prompt — so the
        model treats it as queried ground truth (the anti-hallucination fix)."""
        ...
        state["account_state"] = "当前空仓（持仓 0，现金即全部资产）"
        pm_node(state)

        prompt = captured["prompt"]
        assert isinstance(prompt, list), "account_state 非空时 prompt 应为 message list"
        tool_messages = [m for m in prompt if isinstance(m, ToolMessage)]
        assert len(tool_messages) == 1
        assert "当前空仓" in tool_messages[0].content
        ai_messages = [m for m in prompt if type(m).__name__ == "AIMessage"]
        assert any(getattr(m, "tool_calls", None) for m in ai_messages), \
            "应含 get_account_state 工具调用"
        # DeepSeek thinking mode 400s if a prior assistant tool-call turn lacks
        # reasoning_content; the synthetic get_account_state call must carry it.
        tool_call_msg = next(m for m in ai_messages if getattr(m, "tool_calls", None))
        assert tool_call_msg.additional_kwargs.get("reasoning_content"), \
            "合成 get_account_state 调用必须携带 reasoning_content（DeepSeek 思考模式要求回传）"
