...
# DeepSeek V4 accepts ``reasoning_effort`` (low/high/max) as a real request
# parameter — see api-docs.deepseek.com/guides/thinking_mode. It is not an
# OpenAI-only knob, so V4 models must not have the kwarg stripped.
_DEEPSEEK_REASONING_MODEL = re.compile(r"^deepseek-(v\d+|reasoner)")


def _supports_reasoning_effort(model: str) -> bool:
    """Whether the model accepts a top-level ``reasoning_effort`` parameter."""
    name = model.lower().strip()
    return bool(
        _OPENAI_REASONING_MODEL.match(name) or _DEEPSEEK_REASONING_MODEL.match(name)
    )
...
class OpenAIClient(BaseLLMClient):
    ...
    def get_llm(self) -> Any:
        ...
        # GLM thinking depth: GLM-4.5+ models ship with deep-thinking enabled.
        # Default to 'high' (full depth, best decision quality); flash variants
        # and other latency-sensitive endpoints default to 'low'. Override via
        # TRADINGAGENTS_GLM_THINKING_LEVEL=low/max/off/high. 'off' EXPLICITLY
        # disables thinking ({"type": "disabled"}) — omitting the param would
        # fall back to the provider default, which is thinking ON.
        if self.provider in ("glm", "glm-cn"):
            default_level = "low" if "flash" in self.model.lower() else "high"
            level = os.environ.get("TRADINGAGENTS_GLM_THINKING_LEVEL", default_level).strip().lower()
            if level in ("off", "disabled"):
                llm_kwargs["extra_body"] = {
                    **(llm_kwargs.get("extra_body") or {}),
                ...
                if level in ("low", "high", "max"):
                    ...
                    thinking = {"type": level}
                llm_kwargs["extra_body"] = {**(llm_kwargs.get("extra_body") or {}), "thinking": thinking}
