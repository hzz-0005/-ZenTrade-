...
# The free tier allows ~25 requests per day. Re-running the same US backtest
# window re-burns the same calls (the disk cache was removed), so every request
# is counted here — the single choke point for ALL Alpha Vantage functions
# (news, fundamentals, indicators) in both the webapp and the CLI pipeline —
# and a warning fires as the limit approaches, instead of the quota running out
# silently mid-session and the news feed quietly turning empty.
DAILY_QUOTA_LIMIT = 25
DAILY_QUOTA_WARNING_THRESHOLD = 20

_usage_count = 0
_usage_date: str | None = None


def _record_usage() -> None:
    """Count one outgoing request against today's free-tier quota."""
    ...
    if _usage_date != today:
        ...
        _usage_date = today
    _usage_count += 1
    if _usage_count == DAILY_QUOTA_WARNING_THRESHOLD:
        logger.warning(
            "Alpha Vantage quota: %d/%d requests used today — re-running "
            "already-covered windows re-burns the daily allowance; expect "
            "rate-limit errors if this continues",
            _usage_count, DAILY_QUOTA_LIMIT,
        )
    elif _usage_count >= DAILY_QUOTA_LIMIT:
        logger.warning(
            "Alpha Vantage quota: %d requests today — at/over the free-tier "
            "limit of %d; further calls will be rejected by the API",
            _usage_count, DAILY_QUOTA_LIMIT,
        )


def get_quota_usage() -> tuple[int, str | None]:
    """(requests sent today, today's ISO date) for monitoring/auditing."""
    return _usage_count, _usage_date


def _reset_quota_usage() -> None:
    """Reset the counter (test helper)."""
    global _usage_count, _usage_date
    _usage_count = 0
    _usage_date = date.today().isoformat()
